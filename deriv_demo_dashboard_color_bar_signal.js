...
// Horizontal mini-chart for ticks with last signal highlighted
function generateChart(prices, lastSignalIndex = -1, width = 70) {
    ...
    for (let i = 0; i < prices.length; i++) {
        ...
        let color = p > prev ? colors.green : p < prev ? colors.red : colors.white;
        if (i === lastSignalIndex) color = colors.magenta; // Highlight signal
...
const MAX_TICK_HISTORY = 100; // increased tick history
...
function printDashboard() {
    console.clear();
    console.log(`${colors.cyan}===== VolSim-Pro Color ASCII Dashboard w/ Last Signal Highlight =====${colors.reset}`);
    symbols.forEach(sym => {
        ...
        const chart = generateChart(sd.tickHistory.slice(-MAX_TICK_HISTORY), sd.lastSignalIndex, 70);
        console.log(chart);

        const plBar = generatePLBar(sd.pl);
        console.log(`P/L Trend: ${plBar}`);
