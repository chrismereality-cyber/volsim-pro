...
// Horizontal mini-chart for ticks
...
// Inline P/L bar chart
function generatePLBar(pl, maxLength = 20) {
    const len = Math.min(maxLength, Math.round(Math.abs(pl)));
    const bar = pl >= 0 ? colors.green + '▇'.repeat(len) + colors.reset
                        : colors.red + '▇'.repeat(len) + colors.reset;
...
function printDashboard() {
    console.clear();
    console.log(`${colors.cyan}===== VolSim-Pro Color ASCII Dashboard with P/L Bars =====${colors.reset}`);
    symbols.forEach(sym => {
