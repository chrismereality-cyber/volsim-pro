...
function colorGradient(prev, curr) {
    if (curr > prev) {
        if (curr - prev > 0.05) return colors.green; // strong up
        else return colors.yellow; // mild up
    } else if (curr < prev) {
        if (prev - curr > 0.05) return colors.red; // strong down
        else return colors.yellow; // mild down
    }
    return colors.white; // flat
}
...
function printDashboard() {
    console.clear();
    console.log(`${colors.cyan}===== VolSim-Pro Live Gradient Dashboard =====${colors.reset}`);
    symbols.forEach(sym => {
