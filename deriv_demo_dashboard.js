...
symbols.forEach(sym => {
    symbolData[sym] = {
        ...
        trades: [] // track trades for dashboard
    };
});
...
// Helper: print mini dashboard
function printDashboard() {
    console.clear();
    console.log("===== VolSim-Pro Demo Dashboard =====");
    symbols.forEach(sym => {
        const sd = symbolData[sym];
        console.log(`\n[${sym}] Trades: ${sd.tradeCount}`);
        sd.trades.slice(-5).forEach(t => {
            console.log(`  ${t.type} $${t.price} → payout: ${t.payout} | balance: ${t.balance_after}`);
        });
    });
}
